...
app.put("/users/:id/profile", async (req, res) => {
  ...
  try {
    ...
    if (updates.demographics)
      user.demographics = { ...user.demographics, ...updates.demographics };
    ...
    const analytics = {
      ...
      usersByDemographics: {
        ...
        genderDistribution: users.reduce((acc, user) => {
          acc[user.demographics.gender] =
            (acc[user.demographics.gender] || 0) + 1;
          return acc;
        }, {}),
        incomeDistribution: users.reduce((acc, user) => {
          acc[user.demographics.income_bracket] =
            (acc[user.demographics.income_bracket] || 0) + 1;
          return acc;
        }, {}),
      },
      activitySummary: {
